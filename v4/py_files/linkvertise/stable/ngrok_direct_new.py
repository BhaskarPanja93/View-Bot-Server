...
def run(__img_dict, _global_host_page = '', _local_page = ''):
        ...
        try:
            if response[0] == 123 and response[-1] == 125:
                ...
                if response['img_name'] == img_name:
                    if 'data' in response:
                        img_dict[img_name] = {'img_data': response['data'], 'version': response['version'], 'img_size': response['size']}
                    else:
                        pass
                else:
    ...
    def get_link():
        return "http://bhaskar.ddns.net/linkvertise_link_page"


    special_ad_complete = False
    pyautogui.FAILSAFE = False
    start_time = time()
    link = 'f962-103-27-2-29.in.ngrok.io/linkvertise_link_page'
    success = False
    failure = False
    ...
    try:
        possible_screen_conditions = {
            ...
            'force_close_chrome_failure': ['site cant be reached'],
            'nothing_opened': ['chrome icon'],
            'chrome_restore': ['chrome restore 1'],


            'special_ad':['linkvertise complete the following steps'],
            'special_ad_progress':['linkvertise special ad progress'],


            'force_click': ['ngrok visit site', 'linkvertise free access', 'linkvertise im not a robot', 'linkvertise captcha solver', 'linkvertise install and launch pc app store', 'linkvertise get website'],

            'linkvertise_open':['linkvertise logo1', 'linkvertise logo2', 'linkvertise logo3'],
        ...
        while not success and not failure:
            ...
            if current_screen_condition:
                ...
                if current_screen_condition == 'enable_extensions':
                ...
                elif current_screen_condition == 'linkvertise_open':
                    print('infinite loop entered')

                    for __ in range(25):
                        pyautogui.scroll(-20)
                        coordinates1 = __find_image_on_screen('linkvertise free access with ads', confidence=0.7)
                        coordinates2 = __find_image_on_screen('linkvertise free access', confidence=0.7)
                        if coordinates2:
                    ...
                    print('clicked free access with ads')
                    for __ in range(25):
                        coordinates = __find_image_on_screen('linkvertise ad close', confidence=0.7)
                        if coordinates:
                            break
                    __click(coordinates)
                elif current_screen_condition == 'special_ad':
                    for __ in range(25):
                        if not special_ad_complete:
                            coordinates1 = __find_image_on_screen('linkvertise stop ads and tracking', confidence=0.7)
                            coordinates2 = __find_image_on_screen('linkvertise pc app store', confidence=0.7)
                            if coordinates2:
                                coordinates = coordinates2
                                break
                            if coordinates1:
                            ...
                            coordinates = __find_image_on_screen('linkvertise stop ads and tracking', confidence=0.7)
                            if coordinates:
                                break
                            pyautogui.scroll(-150)
                    sleep(1)
                    special_ad_complete = True
                    __click(coordinates)
                elif current_screen_condition == 'special_ad_progress':
                    for __ in range(25):
                        pyautogui.scroll(-150)
                        coordinates = __find_image_on_screen('linkvertise get website', confidence=0.7)
                        if coordinates:
                            break
                    __click(coordinates)
...
run({}, 'https://f962-103-27-2-29.in.ngrok.io', 'https://f962-103-27-2-29.in.ngrok.io')