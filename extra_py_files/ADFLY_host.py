...
paragraph_lines = open(getcwd()+f'/{read_only_location}/paragraph.txt', 'rb').read().decode().split('.')
youtube_links = open(getcwd()+f'/{read_only_location}/youtube links.txt', 'r').read().split('\n')
    ...
    def acceptor():
        ...
        try:
            if request_code == '-1':
                ...
                if ('user_agents.txt' not in text_files) or (path.getmtime(getcwd() + f'/{read_only_location}/user_agents.txt') != text_files['user_agents.txt']['version']):
                    text_files['user_agents.txt'] = {'version': path.getmtime(getcwd() + f'/{read_only_location}/user_agents.txt'), 'file': open(getcwd() + f'/{read_only_location}/user_agents.txt', 'rb').read()}
