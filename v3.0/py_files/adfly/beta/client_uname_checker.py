...
def run():
    ...
    def try_username_password():
        ...
        while True:
            ...
            response = popen(f"curl -L -s {global_host_page}/request_instance_token?u_name={user_name}&password={password} --max-time 10").read().encode()
            if response[0] == 123 and response[-1] == 125:
                response = eval(response)
                if response['status_code'] == 0:
